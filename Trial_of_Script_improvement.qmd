...
for (sample_name in 1:length(unique_sample_names)) {
  ...
  ######################################################If I prepare combinations in rows 1,2 for SCCPs and 3,4 for MCCPs#############################################
  ####################################################Separating different C1, C2, C3, C4#########################################
...
sample_df <- sample_df  |> 
  mutate(
    ...
    `Normalized Area` = as.numeric(`Normalized Area`),
    `Normalized Area` = replace(`Normalized Area`, is.na(`Normalized Area`), 0),
    Relative_distribution = `Normalized Area` / sum(`Normalized Area`, na.rm = TRUE)
  )

results <- sample_df
results[c("Comp_1", "Comp_2", "Comp_3", "Comp_4", "Fraction_Comp_1", "Simulated_pattern")] <- NA
...
for (z in 1:length(Combinations[1,])) {
  ...
  C_3 <- subset(input, subset = (STD_code == Combinations[3, z]))
  C_4 <- subset(input, subset = (STD_code == Combinations[4, z]))

  sample_list <- split(sample_df, sample_df$Type)
  sample_splited <- unique(sample_df$Type)

  for (g in sample_splited) {
    sample_splited_df <- sample_list[[g]]
    ...
    if (g == 2) {
      for (j in 1:100) {
        Combo2 <- (C_3$Response_factor * j + C_4$Response_factor * (100 - j)) / sum((C_3$Response_factor * j + C_4$Response_factor * (100 - j)), na.rm = TRUE)
        
        if (!any(is.na(REF)) && !any(is.na(Combo2)) && Distance > sum(sqrt((REF - Combo2)^2))) {
          results$Comp_3 <- as.character(C_3$STD_code)
          results$Comp_4 <- as.character(C_4$STD_code)
          results$Fraction_Comp_3 <- j
          results$Simulated_pattern <- Combo2
          Distance <- sum(sqrt((REF - Combo2)^2))
        }
      }
    }
  }
...
for (i in 1:nrow(results)) {
  results$RF_1st[i] <- ifelse(length(input$Sum_response_factor[input$STD_code == results$Comp_1[i]]) > 0, input$Sum_response_factor[input$STD_code == results$Comp_1[i]], NA)
  results$RF_2nd[i] <- ifelse(length(input$Sum_response_factor[input$STD_code == results$Comp_2[i]]) > 0, input$Sum_response_factor[input$STD_code == results$Comp_2[i]], NA)
  results$RF_3rd[i] <- ifelse(length(input$Sum_response_factor[input$STD_code == results$Comp_3[i]]) > 0, input$Sum_response_factor[input$STD_code == results$Comp_3[i]], NA)
  results$RF_4rd[i] <- ifelse(length(input$Sum_response_factor[input$STD_code == results$Comp_4[i]]) > 0, input$Sum_response_factor[input$STD_code == results$Comp_4[i]], NA)
...
results <- results %>%
  mutate(
    ...
    RF_3rd = as.numeric(RF_3rd),
    RF_4rd = as.numeric(RF_4rd),
...
######################################################If I prepare combinations in columns 1:3 for SCCPs and 4:6 for MCCPs#############################################
  ####################################################Separating different C1, C2, C3, C4 AND IF CONDITION #########################################
...
for (g in sample_splited) {
  ...
  results <- sample_df
  results[c("Comp_1", "Comp_2", "Comp_4", "Comp_5", "Fraction_Comp_1", "Simulated_pattern")] <- NA
  ...
  for (z in 1:length(Combinations[1,])) {
    C_1 <- subset(input, subset = (STD_code == Combinations1[1, z]))
    C_2 <- subset(input, subset = (STD_code == Combinations1[2, z]))
 for (j in 1:100) {
     Combo1 <- (C_1$Response_factor * j + C_2$Response_factor * (100 - j)) / sum((C_1$Response_factor * j + C_2$Response_factor * (100 - j)), na.rm = TRUE)
    # Check for NA values in REF or Combo before comparison
     if (!any(is.na(REF)) && !any(is.na(Combo1)) && Distance > (sum(sqrt((REF - Combo1)^2)))) {
        results$Comp_1 <- as.character(C_1$STD_code)
        results$Comp_2 <- as.character(C_2$STD_code)
        results$Fraction_Comp_1 <- j
        results$Simulated_pattern <- Combo1
        Distance <- sum(sqrt((REF - Combo1)^2))
     }
    
  if (g == 2) {
  for (z in 1:length(Combinations[1,])) {
    C_4 <- subset(input, subset = (STD_code == Combinations2[1, z]))
    C_5 <- subset(input, subset = (STD_code == Combinations2[2, z]))
  
    for (j in 1:100) {
      Combo2 <- (C_4$Response_factor * j + C_5$Response_factor * (100 - j)) / sum((C_4$Response_factor * j + C_5$Response_factor * (100 - j)), na.rm = TRUE)
      
      # Check for NA values in REF or Combo before comparison
      if (!any(is.na(REF)) && !any(is.na(Combo2)) && Distance > (sum(sqrt((REF - Combo2)^2)))) {
        results$Comp_4 <- as.character(C_4$STD_code)
        results$Comp_5 <- as.character(C_5$STD_code)
        results$Fraction_Comp_4 <- j
        results$Simulated_pattern <- Combo2
        Distance <- sum(sqrt((REF - Combo2)^2))
        }
       }
...
# Calculate concentrations (ng per microliter)
  ...
  for (i in 1:nrow(results)) {
    ...
    results$RF_3rd[i] <- input$Sum_response_factor[input$STD_code == results$Comp_4[i]]
    results$RF_4rd[i] <- input$Sum_response_factor[input$STD_code == results$Comp_5[i]]
  }
  ...
  results <- results |> 
    mutate(
      ...
      RF_3rd = as.numeric(RF_3rd),
      RF_4rd = as.numeric(RF_4rd))
      results$Concentration <- NA
  if (g == 1) {
    results$Concentration <- sum(results$`Normalized Area`, na.rm = TRUE) / (results$RF_1st * (results$Fraction_Comp_1 / 100) + results$RF_2nd * ((100 - results$Fraction_Comp_1) / 100))
  } else if (g == 2) {
    results$Concentration <- sum(results$`Normalized Area`, na.rm = TRUE) / (results$RF_3rd * (results$Fraction_Comp_4 / 100) + results$RF_4th * ((100 - results$Fraction_Comp_4) / 100))
  }
...
######################################################If I prepare combinations in columns 1:3 for SCCPs and 4:6 for MCCPs#############################################
  ####################################################Separating different  deppeding on conditioning #########################################
 ## The script was made for quantifying one sample at the time, so I changed it to iterate for all the data frames of the samples
...
for (i in 1:length(levels(sample_df$Type))) {
  ...
  for (z in 1:length(Combinations[1, ])) { 
    ...
    C_1 <- subset(input, subset = (STD_code == Combinations[1, z] & Type == levels(sample_df$Type)[i]))
    C_2 <- subset(input, subset = (STD_code == Combinations[2, z] & Type == levels(sample_df$Type)[i]))
...
results$RF_1st[results$Type == levels(results$Chain_length)[1]] <- input$Sum_response_factor[input$STD_code == results$Comp_1 & input$Chain_length == levels(results$Chain_length)[1]]
...
results <- results %>% group_by(Chain_length) %>% mutate(Concentration= sum('Normalized Area')/sum(RF_1st*(Fraction_Comp_1/100)+RF_2nd*((100-Fraction_Comp_1)/100)))
...
working.directory <- "F:/LINKOPING/Manuscripts/Skyline/NIST_Dust/LC-qToF/Skyline/R/"

#####PREPARE DATASET FOR PATTERN RECONSTRUCTION#####
{
#Open input file
input <- read_xlsx(paste(working.directory,"SK_SCCPs_input_ref_LC.xlsx", sep = ""))
...
####Open sample data####
{
sample <- read_xlsx(paste(working.directory,"SCCPs_NISTR1.xlsx", sep = ""))
####Set sample name####
sample.name <- "NISTR1"
####RUN PATTERN RECONSTRACTION FOR SELECTED (LOADED) SAMPLE####
{
sample$Chain_length <- as.factor(sample$Chain_length)
sample$'Normalized Area' <- as.numeric(sample$'Normalized Area')
#CaLCulate relative 'Normalized Area' distribution within each homologue group
sample$Relative_distribution <- NA
sample$'Normalized Area'[is.na(sample$'Normalized Area')] <- 0
sample <- sample %>% group_by(Chain_length) %>% mutate(Relative_distribution = 'Normalized Area'/sum('Normalized Area', na.rm = TRUE))
results <- sample
results[c("Comp_1","Comp_2","Fraction_Comp_1","Simulated_pattern")] <- NA
...
####SAVE RESULTS####
#Set save directory
save.directory <- "F:/LINKOPING/Manuscripts/Skyline/NIST_Dust/LC-qToF/Skyline/R/"
...
#####Set working directory#####
working.directory <- "F:/LINKOPING/Manuscripts/Skyline/NIST_Dust/GC-Orbitrap/Skyline/R/"

#####PREPARE DATASET FOR PATTERN RECONSTRUCTION#####
{
#Open input file
input <- read_xlsx(paste(working.directory,"input_ref_MCCPs_NCI.xlsx", sep = ""))
input$STD_code <- as.factor(input$STD_code)
...
####Open sample data####
sample <- read_xlsx(paste(working.directory,"NISTR2_MCCPs_NCI.xlsx", sep = ""))
####Set sample name####
sample.name <- "NISTR2"
####RUN PATTERN RECONSTRACTION FOR SELECTED (LOADED) SAMPLE####
{
...
sample$Relative_distribution <- NA
sample$'Normalized Area'[is.na(sample$'Normalized Area')] <- 0
sample <- sample %>% mutate(Relative_distribution = 'Normalized Area'/sum('Normalized Area', na.rm = TRUE))
results <- sample
results[c("Comp_1","Comp_2","Fraction_Comp_1","Simulated_pattern")] <- NA
#Deconvolation of homologue patterns
...
results <- results %>% mutate(Concentration= sum('Normalized Area')/(RF_1st*(Fraction_Comp_1/100)+RF_2nd*((100-Fraction_Comp_1)/100)))
...
####SAVE RESULTS####
#Set save directory
save.directory <- "F:/LINKOPING/Manuscripts/Skyline/NIST_Dust/GC-Orbitrap/TraceFinder/R/MCCPs/"
...
#####################Previous trials
 ####RUN PATTERN RECONSTRACTION FOR SELECTED (LOADED) SAMPLE####
{
sample_df$Type <- as.factor(sample_df$Type)
sample_df$`Normalized Area` <- as.numeric(sample_df$`Normalized Area`)
#CaLCulate relative `Normalized Area` distribution within each homologue group
sample_df$Relative_distribution <- NA
sample_df$`Normalized Area`[is.na(sample_df$`Normalized Area`)] <- 0
sample_df <- sample_df |> 
...
for (i in 1:length(levels(sample_df$Type))) {
REF <- sample_df$Relative_distribution[sample_df$Type == levels(input$Type)[i]]
Distance <- 100
for (z in 1:length(Combinations[1,])){                                     
C_1 <- subset(input,subset = (STD_code == Combinations[1,z] & Type == levels(sample_df$Type)[i]))
C_2 <- subset(input,subset = (STD_code == Combinations[2,z] & Type == levels(sample_df$Type)[i]))
for (j in 1:100) {
  ...
  if (Distance > (sum(sqrt((REF-Combo)^2)))) {
    ...
    results$Comp_2[results$Type == levels(sample_df$Type)[i]] <- as.character(C_2$STD_code)
    results$Fraction_Comp_1 <- j
    results$Simulated_pattern[results$Type == levels(input$Type)[i]] <- Combo
    Distance <- sum(sqrt((REF-Combo)^2))
  }
}
}
...
for (g in sample_splited) {
  ...
  if (g == 2) {  
      Combo <- (C_3$Response_factor * j + C_4$Response_factor * (100 - j)) / sum((C_3$Response_factor * j + C_4$Response_factor * (100 - j)), na.rm = TRUE)}
  ...
  for (i in 1:nrow(results)) {
    results$RF_1st[i] <- input$Sum_response_factor[input$STD_code == results$Comp_1[i]]
    results$RF_2nd[i] <- input$Sum_response_factor[input$STD_code == results$Comp_2[i]]
    results$RF_3rd[i] <- input$Sum_response_factor[input$STD_code == results$Comp_3[i]]
    results$RF_4rd[i] <- input$Sum_response_factor[input$STD_code == results$Comp_4[i]]
  }
